...
async function probe(path: string): Promise<boolean> {
  try {
    ...
    // 403 = catch-all rejection (e.g. Codex endpoint rejects unknown paths).
    // 401 = agent requires auth but our token is missing or wrong — treat as
    //       unreachable so the UI shows disconnected rather than silently failing.
    // Only 2xx, 400, 405, 422 reliably indicate the endpoint exists and is usable.
...
async function probeChatCompletions(): Promise<boolean> {
  try {
    ...
    // 401/403 = auth required or rejected — treat as unusable
    if (getRes.status === 401 || getRes.status === 403) return false
